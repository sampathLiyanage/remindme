...
class Events_DB extends DB_connection{
	...
	*/
	public function createSchedule($userId, $title, $description){
		
		$stmt = $this->prepareSqlStmt( "INSERT INTO schedule (user_id, title, description, date_created, date_updated) 
	...
	*@output=> if query changed the database:bool
	*/
	public function changeSchedule($id, $title, $description){
	...
	*/
	public function getSchReminderUser($reminderId){
	        $stmt = $this->prepareSqlStmt( "SELECT user_id FROM schedule, schedule_event, sch_event_reminder
	                                        WHERE sch_event_reminder.id=?
	                                        AND sch_event_reminder.sch_event_id=schedule_event.id
