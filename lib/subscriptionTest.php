...
class TdListSubscribeHandlerTest extends PHPUnit_Framework_TestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$tdListId = $row[0];
        //publish the Remind list
        $rMan           = new RemindListManager(self::$userId);
        $result         = $rMan->publishRemindList(self::$tdListId);
        self::assertTrue($result);
        $sMan      = new SubscribeHandler(self::$userId);
        $key       = $sMan->getKeyFromId(self::$tdListId);
        self::$key = $key;
        self::assertTrue($key != null);
        self::assertTrue(strlen($key) == 10);
    }
    ...
    public function testRemindSubcriptions()
    {
        ...
        self::$scrMan = new SubscribeHandler(self::$userId);
        $id           = self::$tdListId;
        //subcribe RemindList;
        $r            = self::$scrMan->subscribe(self::$key);
        $this->assertTrue($r);
        //get subciptions by userId
        $r = self::$scrMan->getSubscribedLists();
